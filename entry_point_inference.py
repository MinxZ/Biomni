...
def parse_args():
    ...
    parser.add_argument('--model_path', type=str, help='S3 link for the model')
    parser.add_argument('--link1', type=str, help='S3 data link')
    parser.add_argument('--link2', type=str, help='S3 data link')
...
if __name__ == "__main__":
    ...
    link = args.link1 + args.link2
    job_name = args.job_name
    model_path = args.model_path

    df = pd.read_csv(link)
    smiles = df['_id']  # SMILES strings of molecules

    f = "temp.joblib"
    s3_client.download_file(bucket_name, model_path, f)
    ...
    print(y_pred)
    stat_file = f'{job_name}-predict_results.json'
